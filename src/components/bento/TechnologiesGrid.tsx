...
import TechnologiesCard from '../ui/TechonologiesCard';

export default function TechnologiesGrid() {
  const frontEndTechs = TECHNOLOGIES_DATA.filter((t) =>
    ["JavaScript", "TypeScript", "Next.js", "HTML5", "CSS3", "Tailwind", "React", "Sass"].includes(t.id)
  );
  ...
  return (
    <section className="w-full max-w-6xl mx-auto p-4 flex flex-col gap-12">
          ...
          <div className="grid grid-cols-3 auto-rows-[120px] gap-4 w-full">
            {frontEndTechs.map((tech) => (
              <div key={tech.id} className={tech.gridSpan}>
                <TechnologiesCard color={tech.color} title={tech.id} img={tech.img} />
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Sección Opcional: Otras Tecnologías y Herramientas */}
      {otherTechs.length > 0 && (
